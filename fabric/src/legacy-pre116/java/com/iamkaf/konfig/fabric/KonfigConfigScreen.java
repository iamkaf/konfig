...
public final class KonfigConfigScreen extends Screen {
    ...
    private static final LegacyValueText.Translator<Component> VALUE_TEXT_TRANSLATOR = new LegacyValueText.Translator<Component>() {
        @Override
        ...
        @Override
        public Component translationOrNull(String key) {
            return KonfigConfigScreen.translationOrNull(key);
        }

        @Override
        public String string(Component value) {
    ...
    public void render(int mouseX, int mouseY, float partialTick) {
        ...
        try {
            ...
            if (this.renderedDropdownRow != null) {
                this.renderedDropdownRow.renderDropdown(mouseX, mouseY);
            }
    ...
    private void queueTooltip(String tooltip, int mouseX, int mouseY) {
        if (isBlank(tooltip)) {
            return;
        }
        this.queueTooltip(tooltipLines(tooltip), mouseX, mouseY);
    }
    ...
    private void renderInfoPanel(int mouseX, int mouseY) {
        ...
        for (InfoPanelItem item : items) {
            y = this.renderInfoPanelItem(item, x, y, contentWidth, mouseX, mouseY);
        }
        this.disableInfoPanelScissor();

        this.renderInfoPanelScrollbar(right, viewportTop, viewportBottom);
    }

    private int renderInfoPanelItem(InfoPanelItem item, int x, int y, int width, int mouseX, int mouseY) {
        if (item.kind == EntryKind.HEADER) {
            this.font.draw(infoText(item), x, y, 0xFFFFFFFF);
            return y + 16;
        }
        if (item.kind == EntryKind.IMAGE) {
            return this.renderInfoImage(item, x, y, width);
        }
        if (item.kind == EntryKind.URL) {
            String label = infoText(item) + " >";
        ...
        return this.renderInfoParagraph(infoText(item), x, y, width, 0xFFCFCFCF) + INFO_PANEL_GAP;
    }

    private int renderInfoImage(InfoPanelItem item, int x, int y, int width) {
        ...
        if (!isBlank(infoText(item)) && options.captionPosition() != ImageOptions.CaptionPosition.NONE) {
            y = this.renderInfoParagraph(infoText(item), x, y, width, 0xFFCFCFCF);
        }
    ...
    private void renderInfoPanelScrollbar(int right, int top, int bottom) {
        ...
        GuiComponent.fill(trackLeft, top, trackRight, bottom, 0x44000000);
        GuiComponent.fill(trackLeft, thumbTop, trackRight, thumbTop + thumbHeight, 0xAAFFFFFF);
    }

    private void enableInfoPanelScissor(int left, int top, int right, int bottom) {
        Object window = minecraftWindow();
        double scale = windowGuiScale(window);
        int windowHeight = windowHeight(window);
        int scissorX = (int) Math.round(left * scale);
        int scissorY = (int) Math.round(windowHeight - (bottom * scale));
    ...
    private Object minecraftWindow() {
        try {
            return Minecraft.class.getMethod("getWindow").invoke(this.minecraft);
        } catch (Exception ignored) {
            try {
                java.lang.reflect.Field windowField = Minecraft.class.getDeclaredField("window");
                windowField.setAccessible(true);
                return windowField.get(this.minecraft);
            } catch (Exception exception) {
                throw new IllegalStateException("Unable to access Minecraft window", exception);
            }
        }
    }

    private static double windowGuiScale(Object window) {
        try {
            return ((Number) window.getClass().getMethod("getGuiScale").invoke(window)).doubleValue();
        } catch (Exception exception) {
            throw new IllegalStateException("Unable to read Minecraft GUI scale", exception);
        }
    }

    private static int windowHeight(Object window) {
        try {
            return ((Number) window.getClass().getMethod("getHeight").invoke(window)).intValue();
        } catch (Exception exception) {
            throw new IllegalStateException("Unable to read Minecraft window height", exception);
        }
    }
    ...
    private Component dropdownValueText(EntryRef entry, String option) {
        return LegacyValueText.dropdownValueText(entry.legacyEntry, option, VALUE_TEXT_TRANSLATOR);
    }

    private Component translatedDropdownOption(EntryRef entry, DropdownOptionMetadata option) {
    ...
    private static void drawColorSwatch(int x, int y, int size, int color, EntryKind kind) {
        GuiComponent.fill(x - 1, y - 1, x + size + 1, y + size + 1, 0xFF202020);
        if (kind == EntryKind.COLOR_ARGB && ColorValueHelper.alpha(color) < 255) {
            ...
            GuiComponent.fill(x, y, x + size, y + size, 0xFFFFFFFF);
        }
        GuiComponent.fill(x, y, x + size, y + size, ColorValueHelper.toRenderColor(kind, color));
    }
    ...
    private static void renderRegistryIcon(String registryId, String value, int x, int y) {
    ...
    private static List<String> tooltipLines(String tooltip) {
        List<String> lines = new ArrayList<String>();
    ...
    private final class EntryList extends ContainerObjectSelectionList<ConfigRow> {
        ...
        protected void renderBackground() {
            GuiComponent.fill(this.x0, this.y0, this.x1, this.y1, 0x66000000);
        }
    }
    ...
    private abstract class ConfigRow extends ContainerObjectSelectionList.Entry<ConfigRow> {
        ...
        @Override
        public void render(int index, int y, int x, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            this.renderRow(x, y, width, height, mouseX, mouseY, hovered, partialTick);
        }
        ...
        protected void renderRow(int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            ...
            if (!isBlank(rowTooltip)) {
                if (mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + height) {
                    KonfigConfigScreen.this.queueTooltip(rowTooltip, mouseX, mouseY);
                }
            }
            ...
            KonfigConfigScreen.this.font.draw(this.entry.contextLabel.getString(), x + 4.0F, y + 1.0F, 0xFFA0A0A0);
            KonfigConfigScreen.this.font.draw(this.entry.displayLabel().getString(), x + 4.0F, y + 12.0F, this.entry.editable ? 0xFFFFFFFF : 0xFFA0A0A0);
            this.control().render(mouseX, mouseY, partialTick);
            if (!this.validationMessage().isEmpty()) {
                KonfigConfigScreen.this.font.draw(this.validationMessage(), controlX, controlY + CONTROL_HEIGHT + 2, VALIDATION_COLOR);
            }
        }
    ...
    private final class UnsupportedRow extends ConfigRow {
        ...
        private UnsupportedRow(EntryRef entry) {
            super(entry);
            this.button = new Button(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, translate("konfig.screen.unsupported").getString(), ignored -> {
            });
    ...
    private final class BooleanRow extends ConfigRow {
        ...
        private BooleanRow(EntryRef entry) {
            super(entry);
            this.button = new Button(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, booleanText(entry.value).getString(), button -> {
        ...
        @Override
        protected void syncFromDraft() {
            this.button.setMessage(booleanText(this.entry.value).getString());
        }
    }
    ...
    private final class EnumRow extends ConfigRow {
        ...
        private EnumRow(EntryRef entry) {
            super(entry);
            this.button = new Button(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, enumText(entry, KonfigConfigScreen.this.currentEnum(entry.value)).getString(), button -> {
                Object previousDraft = KonfigConfigScreen.this.draftSession.draft(entry.value);
        ...
        @Override
        protected void syncFromDraft() {
            this.button.setMessage(enumText(this.entry, KonfigConfigScreen.this.currentEnum(this.entry.value)).getString());
        }
    }
    ...
    private final class DropdownRow extends ConfigRow {
        ...
        @Override
        protected void renderRow(int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            super.renderRow(x, y, width, height, mouseX, mouseY, hovered, partialTick);
        ...
        private void renderButtonLabel() {
            ...
            KonfigConfigScreen.this.font.draw(valueText.getString(), (float) textX, (float) textY, 0xFFFFFFFF);

            Component chevron = text(this.dropdownState.open() ? "\u25B4" : "\u25BE");
            int chevronX = chevronLeft + Math.max(0, (DROPDOWN_CHEVRON_WIDTH - KonfigConfigScreen.this.font.width(chevron.getString())) / 2);
            KonfigConfigScreen.this.font.draw(chevron.getString(), (float) chevronX, (float) textY, this.dropdownState.open() ? 0xFFF8E38F : 0xFFCFCFCF);
        }
        ...
        private Component fitDropdownText(Component value, int maxWidth) {
            ...
            if (KonfigConfigScreen.this.font.width(value.getString()) <= maxWidth) {
                return value;
            }
        ...
        private void renderDropdown(int mouseX, int mouseY) {
            List<String> options = this.options();
            if (!this.dropdownState.open() || options.isEmpty()) {
                return;
            }
            ...
            this.layoutDropdown();
            GuiComponent.fill(this.lastDropdownX - 1, this.lastDropdownY - 1, this.lastDropdownX + this.lastDropdownWidth + 1, this.lastDropdownY + this.lastDropdownHeight + 1, 0xFF202020);
            GuiComponent.fill(this.lastDropdownX, this.lastDropdownY, this.lastDropdownX + this.lastDropdownWidth, this.lastDropdownY + this.lastDropdownHeight, 0xFF101010);
            ...
            for (int visibleIndex = 0; visibleIndex < visibleCount; visibleIndex++) {
                ...
                if (rowHovered || focused || current) {
                    ...
                    GuiComponent.fill(this.lastDropdownX + 1, rowY, this.lastDropdownX + this.lastDropdownWidth - 1, rowBottom, color);
                }
                if (current) {
                    GuiComponent.fill(this.lastDropdownX + 2, rowY + 2, this.lastDropdownX + 4, rowBottom - 2, 0xFFF8E38F);
                }
                ...
                KonfigConfigScreen.this.font.draw(this.fitDropdownText(dropdownText(this.entry, options.get(optionIndex)), Math.max(0, textRight - textX)).getString(), (float) textX, (float) (rowY + 3), 0xFFFFFFFF);
            }
            ...
            if (this.maxScrollOffset() > 0) {
                ...
                GuiComponent.fill(this.lastDropdownX + this.lastDropdownWidth - 4, trackTop, this.lastDropdownX + this.lastDropdownWidth - 2, trackBottom, 0x44000000);
                GuiComponent.fill(this.lastDropdownX + this.lastDropdownWidth - 4, thumbTop, this.lastDropdownX + this.lastDropdownWidth - 2, thumbTop + thumbHeight, 0xAAFFFFFF);
            }
        }
    }
    ...
    private final class ColorRow extends ConfigRow {
        ...
        private ColorRow(EntryRef entry) {
            super(entry);
            this.button = new Button(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, colorText(entry.value).getString(), ignored -> {
        ...
        @Override
        protected void syncFromDraft() {
            this.button.setMessage(colorText(this.entry.value).getString());
        }
        ...
        protected void renderRow(int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            ...
            KonfigConfigScreen.this.font.draw(this.entry.contextLabel.getString(), x + 4.0F, y + 1.0F, 0xFFA0A0A0);
            KonfigConfigScreen.this.font.draw(this.entry.displayLabel().getString(), x + 4.0F, y + 12.0F, 0xFFFFFFFF);
            drawColorSwatch(previewX, previewY, PREVIEW_SIZE, KonfigConfigScreen.this.currentColor(this.entry.value), this.entry.value.kind());
            this.control().render(mouseX, mouseY, partialTick);
        }
    }
    ...
    private final class StringListRow extends ConfigRow {
        ...
        private StringListRow(EntryRef entry) {
            super(entry);
            this.button = new Button(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, stringListText(entry.value).getString(), ignored -> {
        ...
        @Override
        protected void syncFromDraft() {
            this.button.setMessage(stringListText(this.entry.value).getString());
        }
        ...
        protected void renderRow(int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            ...
            renderRegistryIcon(this.entry.value.boundRegistryId(), values.get(0), previewX, previewY);
        }
    }

    private abstract class BaseSliderWidget extends AbstractSliderButton {
        private BaseSliderWidget(double initialProgress) {
            super(0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, initialProgress);
        }
    ...
    private final class IntegerSliderRow extends ConfigRow {
        ...
        private final class SliderWidget extends BaseSliderWidget {
            ...
            protected void updateMessage() {
                this.setMessage(Integer.toString(IntegerSliderRow.this.currentValue()));
            }
    ...
    private final class LongSliderRow extends ConfigRow {
        ...
        private final class SliderWidget extends BaseSliderWidget {
            ...
            @Override
            protected void updateMessage() {
                this.setMessage(Long.toString(LongSliderRow.this.currentValue()));
            }
    ...
    private final class DoubleSliderRow extends ConfigRow {
        ...
        private final class SliderWidget extends BaseSliderWidget {
            ...
            @Override
            protected void updateMessage() {
                this.setMessage(formatDouble(DoubleSliderRow.this.currentValue()));
            }
    ...
    private final class RegistryTextInputRow extends ConfigRow {
        ...
        private RegistryTextInputRow(EntryRef entry) {
            super(entry);
            this.input = new EditBox(KonfigConfigScreen.this.font, 0, 0, CONTROL_MIN_WIDTH, CONTROL_HEIGHT, entry.label.getString());
        ...
        private void renderSuggestions(int mouseX, int mouseY) {
            ...
            for (int index = 0; index < this.visibleSuggestions.size(); index++) {
                ...
                if (hovered || index == this.selectedSuggestionIndex) {
                    GuiComponent.fill(this.lastDropdownX + 1, rowY, this.lastDropdownX + this.lastDropdownWidth - 1, rowBottom, hovered ? 0x80406080 : 0x50303030);
                }
                int textX = this.lastDropdownX + 4;
                if (this.entry.value.hasBoundRegistry() && supportsRegistryIcon(this.entry.value.boundRegistryId())) {
                    renderRegistryIcon(this.entry.value.boundRegistryId(), this.visibleSuggestions.get(index), this.lastDropdownX + 2, rowY - 1);
                    textX += 18;
                }
                KonfigConfigScreen.this.font.draw(this.visibleSuggestions.get(index), (float) textX, (float) (rowY + 3), 0xFFFFFFFF);
            }
        }
    ...
    private abstract class EntryEditorScreen extends Screen {
        ...
        protected final void renderEditorChrome(int mouseX, int mouseY, float partialTick) {
            GuiComponent.fill(0, 0, this.width, this.height, 0xC0101010);
            super.render(mouseX, mouseY, partialTick);
            drawCenteredString(this.font, this.title.getString(), this.width / 2, EDITOR_TITLE_Y, 0xFFFFFFFF);
            this.font.draw(this.entry.contextLabel.getString(), 12, EDITOR_CONTEXT_Y, 0xFFA0A0A0);
        }
    }
    ...
    private final class ColorEditorScreen extends EntryEditorScreen {
        ...
        protected void init() {
            ...
            this.hexInput = this.addButton(new EditBox(this.font, this.width / 2 - HEX_WIDTH / 2, HEX_Y, HEX_WIDTH, 20, this.entry.label.getString()));
            ...
            int footerY = this.height - 26;
            this.addButton(new Button(this.width / 2 - 122, footerY, 80, 20, translate("konfig.screen.reset").getString(), ignored -> {
                if (this.resetToSessionStart()) {
                    this.syncWidgetsFromDraft();
                }
            }));
            this.addButton(new Button(this.width / 2 + 42, footerY, 80, 20, translate("konfig.screen.done").getString(), ignored -> this.onClose()));
        ...
        @Override
        public void render(int mouseX, int mouseY, float partialTick) {
            this.renderEditorChrome(mouseX, mouseY, partialTick);
            int previewX = this.width / 2 - PREVIEW_SIZE / 2;
            drawColorSwatch(previewX, PREVIEW_Y, PREVIEW_SIZE, KonfigConfigScreen.this.currentColor(this.entry.value), this.entry.value.kind());
        ...
        private void renderValidationMessage() {
            if (!this.validationMessage.isEmpty()) {
                drawCenteredString(this.font, this.validationMessage, this.width / 2, HEX_Y + CONTROL_HEIGHT + 3, VALIDATION_COLOR);
            }
        }
        ...
        private final class ChannelSlider extends BaseSliderWidget {
            ...
            @Override
            protected void updateMessage() {
                this.setMessage(translate(this.channel.translationKey, Integer.valueOf(ColorEditorScreen.this.currentChannel(this.channel))).getString());
            }
    ...
    private final class StringListEditorScreen extends EntryEditorScreen {
        ...
        public void render(int mouseX, int mouseY, float partialTick) {
            ...
            this.font.draw(count, this.width - 12 - this.font.width(count), EDITOR_CONTEXT_Y, 0xFFC0C0C0);
            if (KonfigConfigScreen.this.currentStringList(this.entry.value).isEmpty()) {
                drawCenteredString(this.font, translate("konfig.screen.list.empty").getString(), this.width / 2, this.height / 2 - 12, 0xFFC0C0C0);
            }
            if (this.renderedRegistryRow != null) {
                this.renderedRegistryRow.renderSuggestions(mouseX, mouseY);
            }
        ...
        private void rebuildEditorWidgets() {
            ...
            int footerY = this.height - 26;
            this.addButton(new Button(this.width / 2 - 122, footerY, 80, 20, translate("konfig.screen.list.add").getString(), ignored -> this.addValue()));
            this.addButton(new Button(this.width / 2 - 40, footerY, 80, 20, translate("konfig.screen.reset").getString(), ignored -> {
                if (this.resetToSessionStart()) {
                    this.rebuildEditorWidgets();
                }
            }));
        ...
        private final class ListEntryRow extends ContainerObjectSelectionList.Entry<ListEntryRow> {
            ...
            private ListEntryRow(int index, String value) {
                this.index = index;
                this.input = new EditBox(StringListEditorScreen.this.font, 0, 0, 140, CONTROL_HEIGHT, StringListEditorScreen.this.entry.label.getString());
                this.input.setMaxLength(256);
                this.input.setValue(value);
                this.input.setResponder(this::onValueChanged);

                this.moveUpButton = new Button(0, 0, 20, 20, "^", ignored -> this.move(-1));
                this.moveDownButton = new Button(0, 0, 20, 20, "v", ignored -> this.move(1));
                this.removeButton = new Button(0, 0, 20, 20, "-", ignored -> this.remove());
            }
            ...
            public void render(int index, int y, int x, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
                if (hovered) {
                    GuiComponent.fill(x, y, x + width, y + ITEM_ROW_HEIGHT, 0x22000000);
                }
                ...
                if (this.hasRegistryBinding() && supportsRegistryIcon(this.registryKey())) {
                    renderRegistryIcon(this.registryKey(), this.input.getValue(), x, y + (ITEM_ROW_HEIGHT - ICON_SIZE) / 2);
                ...
                this.input.render(mouseX, mouseY, partialTick);
                this.moveUpButton.render(mouseX, mouseY, partialTick);
                this.moveDownButton.render(mouseX, mouseY, partialTick);
            ...
            private void renderSuggestions(int mouseX, int mouseY) {
                ...
                for (int suggestionIndex = 0; suggestionIndex < this.visibleSuggestions.size(); suggestionIndex++) {
                    ...
                    if (suggestionHovered || suggestionIndex == this.selectedSuggestionIndex) {
                        GuiComponent.fill(this.lastDropdownX + 1, rowY, this.lastDropdownX + this.lastDropdownWidth - 1, rowBottom, suggestionHovered ? 0x80406080 : 0x50303030);
                    }
                    int textX = this.lastDropdownX + 4;
                    if (supportsRegistryIcon(this.registryKey())) {
                        renderRegistryIcon(this.registryKey(), this.visibleSuggestions.get(suggestionIndex), this.lastDropdownX + 2, rowY - 1);
                        textX += 18;
                    }
                    StringListEditorScreen.this.font.draw(this.visibleSuggestions.get(suggestionIndex), (float) textX, (float) (rowY + 3), 0xFFFFFFFF);
                }
            }
