...
@ApiStatus.Internal
// Fabric 1.16.x legacy config screen. Modern Fabric excludes this file in
// fabric/build.gradle, and 1.14/1.15 keep a separate no-PoseStack shell.
public final class KonfigConfigScreen extends Screen {
    ...
    public void render(PoseStack guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        GuiComponent.fill(guiGraphics, 0, this.height - LIST_BOTTOM_MARGIN, this.mainPanelRight(), this.height, 0xC0101010);
    ...
    private void queueTooltip(PoseStack guiGraphics, String tooltip, int mouseX, int mouseY) {
        if (isBlank(tooltip)) {
            return;
        }
//? if <=1.16.1 {
        this.queueTooltip(guiGraphics, tooltipLines(tooltip), mouseX, mouseY);
//?} elif <=1.16.3 {
        this.queueTooltip(guiGraphics, this.font.split(text(tooltip), Math.max(this.width / 2, 200)), mouseX, mouseY);
//?} else {
        this.queueTooltip(guiGraphics, tooltipLines(tooltip), mouseX, mouseY);
//?}
    }
...
//?}
    ...
    private void renderInfoPanel(PoseStack guiGraphics, int mouseX, int mouseY) {
        ...
        GuiComponent.fill(guiGraphics, left, top, right, bottom, 0x22000000);
    ...
    private int renderInfoPanelItem(PoseStack guiGraphics, InfoPanelItem item, int x, int y, int width, int mouseX, int mouseY) {
        ...
        if (item.kind == EntryKind.URL) {
            ...
            if (hovered) {
                GuiComponent.fill(guiGraphics, x, y + this.font.lineHeight, x + Math.min(width, linkWidth), y + this.font.lineHeight + 1, 0xFFFFFFFF);
            }
            return y + 16;
        }
    ...
    private void renderInfoPanelScrollbar(PoseStack guiGraphics, int right, int top, int bottom) {
        ...
        GuiComponent.fill(guiGraphics, trackLeft, top, trackRight, bottom, 0x44000000);
        GuiComponent.fill(guiGraphics, trackLeft, thumbTop, trackRight, thumbTop + thumbHeight, 0xAAFFFFFF);
    }
    ...
    private static void drawImage(PoseStack guiGraphics, String target, int x, int y, int width, int height) {
        drawImage(guiGraphics, target, x, y, width, height, width, height);
    }

    private static void drawImage(PoseStack guiGraphics, String target, int x, int y, int width, int height, int sourceWidth, int sourceHeight) {
...
//?}
    ...
    private static void renderRegistryIcon(PoseStack guiGraphics, String registryId, String value, int x, int y) {
...
//?}
    ...
    private abstract class DecorationRow extends ConfigRow {
        ...
        private DecorationRow(EntryRef entry) {
            super(entry);
            this.spacer = new Button(0, 0, 0, 0, TextComponent.EMPTY, ignored -> {});
...
//?}
    ...
    private final class ImageRow extends DecorationRow {
        ...
        protected void renderRow(PoseStack guiGraphics, int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick) {
            KonfigConfigScreen.this.updateHoveredEntry(this.entry, hovered);
            if (hovered) {
                GuiComponent.fill(guiGraphics, x, y, x + width, y + height, 0x16000000);
            }
            if (!isBlank(this.entry.tooltip) && mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + height) {
//? if <=1.16.1 {
                KonfigConfigScreen.this.queueTooltip(guiGraphics, tooltipLines(this.entry.tooltip), mouseX, mouseY);
...
//?}
    ...
    private final class DropdownRow extends ConfigRow {
        ...
        private void renderDropdown(PoseStack guiGraphics, int mouseX, int mouseY) {
            ...
            GuiComponent.fill(guiGraphics, this.lastDropdownX, this.lastDropdownY, this.lastDropdownX + this.lastDropdownWidth, this.lastDropdownY + this.lastDropdownHeight, 0xFF101010);
            ...
            for (int visibleIndex = 0; visibleIndex < visibleCount; visibleIndex++) {
                ...
                if (rowHovered || focused || current) {
                    ...
                    GuiComponent.fill(guiGraphics, this.lastDropdownX + 1, rowY, this.lastDropdownX + this.lastDropdownWidth - 1, rowBottom, color);
                }
                if (current) {
                    GuiComponent.fill(guiGraphics, this.lastDropdownX + 2, rowY + 2, this.lastDropdownX + 4, rowBottom - 2, 0xFFF8E38F);
                }
            ...
            if (this.maxScrollOffset() > 0) {
                ...
                GuiComponent.fill(guiGraphics, this.lastDropdownX + this.lastDropdownWidth - 4, trackTop, this.lastDropdownX + this.lastDropdownWidth - 2, trackBottom, 0x44000000);
                GuiComponent.fill(guiGraphics, this.lastDropdownX + this.lastDropdownWidth - 4, thumbTop, this.lastDropdownX + this.lastDropdownWidth - 2, thumbTop + thumbHeight, 0xAAFFFFFF);
            }
        }
    }
...
//?}
    ...
    private final class ColorEditorScreen extends EntryEditorScreen {
        ...
        protected void init() {
            ...
            this.hexInput = this.addButton(new EditBox(this.font, this.width / 2 - HEX_WIDTH / 2, HEX_Y, HEX_WIDTH, 20, this.entry.label));
