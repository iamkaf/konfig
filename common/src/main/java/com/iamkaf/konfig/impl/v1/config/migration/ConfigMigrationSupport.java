package com.iamkaf.konfig.impl.v1.config.migration;
...
public final class ConfigMigrationSupport {
    public static final String METADATA_ROOT = "__konfig";
    public static final String VERSION_PATH = METADATA_ROOT + ".version";
    public static final String METADATA_COMMENT = "Konfig internal metadata. Safe to ignore.";

    private ConfigMigrationSupport() {
    }

    public static void validateSchemaVersion(int version) {
        if (version < 0) {
            throw new IllegalArgumentException("schemaVersion must be >= 0: " + version);
        }
    }

    public static boolean isReservedRootSegment(String segment) {
        return METADATA_ROOT.equals(segment);
    }

    public static String requireUserPath(String path, String name) {
    ...
    public static int readSchemaVersion(CommentedConfig root) {
    ...
    public static void writeSchemaVersion(CommentedConfig root, int version) {
        validateSchemaVersion(version);
